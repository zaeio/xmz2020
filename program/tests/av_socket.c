#include <stdio.h>
#include <fcntl.h>
#include <errno.h>
#include <stdlib.h>
#include <unistd.h>
#include <string.h>
#include <stdbool.h>
#include <strings.h>
#include <sys/stat.h>
#include <sys/wait.h>
#include <sys/types.h>
#include <pthread.h>
#include <semaphore.h>
#include <arpa/inet.h>
#include <sys/socket.h>

#include "fbutils.h"

#define IMG_WIDTH 608
#define IMG_HEIGHT 600

extern int send_sound_flag;
extern int recv_sound_flag;
int socket_fd;
struct sockaddr_in servaddr; //服务器ip

//建立语音的TCP连接
void setup_tcp()
{

        // １，创建TCP套接字
        socket_fd = socket(AF_INET, SOCK_STREAM, 0);
        if (socket_fd == -1)
        {
                perror("create sound tcp failed");
                return;
        }

        servaddr.sin_family = AF_INET;
        servaddr.sin_addr.s_addr = inet_addr("192.168.1.116");
        servaddr.sin_port = htons(53000);
        // ２，向服务器发起连接请求
        if (connect(socket_fd, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0)
        {
                // perror("connect falied");
                printf("connect falied\n");
                // close(socket_fd);
        }
}
void send_Vframe(char *graymap, int length)
{
        send(socket_fd, graymap, length, 0);
}

void send_pcm()
{
        char *pcmfilename = "/mnt/frame/ak_ao_test.pcm";
        FILE *fp_pcm = NULL;
        struct stat filestat;
        char *pcmbuf;

        fp_pcm = fopen(pcmfilename, "r");
        if (NULL == fp_pcm)
        {
                printf("open file error\n");
                return;
        }

        stat(pcmfilename, &filestat); //获取文件状态，主要是大小
        pcmbuf = (char *)calloc(filestat.st_size, sizeof(char));
        fread(pcmbuf, sizeof(char), filestat.st_size, fp_pcm);

        send(socket_fd, pcmbuf, filestat.st_size, 0);
        printf("send size = %d\n", filestat.st_size);
        close(socket_fd);
}

void recv_Vframe_routine()
{
        int width = 608;
        int height = 600;
        // １，创建TCP套接字
        int serv_fd = socket(AF_INET /*IPv4*/, SOCK_STREAM /*TCP*/, 0);
        char buf[2000] = {0};
        int ret;
        struct sockaddr_in servaddr;
        socklen_t len = sizeof(servaddr);

        servaddr.sin_family = AF_INET;
        servaddr.sin_addr.s_addr = inet_addr("192.168.1.116"); // 绑定本机IP
        servaddr.sin_port = htons(53000);                      // 端口号，范围: 1 - 65535，最好用50000以上的端口防冲突

        // ２，绑定地址（IP + PORT）
        if (bind(serv_fd, (struct sockaddr *)&servaddr, len) == -1)
        {
                perror("bind sound tcp fail");
                close(serv_fd);
                return NULL;
        }
        // ３，将套接字设置为监听状态
        // 设定之后的fd，就是监听套接字，专用来等待连接的
        if (listen(serv_fd, 1 /*设定最大同时连接个数*/) == -1)
        {
                perror("listen fail");
                close(serv_fd);
                return NULL;
        }
        // ４，坐等对方的连接请求... ...
        // 返回的connfd，就是已连接套接字，专用来读写通信的
        struct sockaddr_in connaddr;
        len = sizeof(connaddr);
        bzero(&connaddr, len);

        int connfd = accept(serv_fd, (struct sockaddr *)&connaddr /*获取对方的地址信息*/, &len);
        //接收对方发来的语音消息，
        if (connfd > 0)
        {
                printf("new connection: [%s:%hu]\n", inet_ntoa(connaddr.sin_addr), ntohs(connaddr.sin_port));
        }
        else
        {
                perror("accept() failed");
                close(connfd);
                close(serv_fd);
                return NULL;
        }

        int recvlen = 0;
        int recvcount = 0;
        char graymap_buf[IMG_WIDTH * IMG_HEIGHT] = {0}; //364800
        while ((recvlen = read(connfd, buf, 2000)) > 0)
        {
                int i, j;
                //复制接收的数据
                for (i = 0; i < recvlen && (recvcount + i) < (IMG_WIDTH * IMG_HEIGHT); i++)
                {
                        graymap_buf[recvcount + i] = buf[i];
                }
                recvcount += recvlen;
                printf("recvlen = %d    recvcount = %d\n", recvlen, recvcount);
                //graymap接收完毕
                if (buf[recvlen - 1] == 0x03 && buf[recvlen - 2] == 0x02 && buf[recvlen - 3] == 0x01)
                {
                        put_gray_map(0, 0, graymap_buf, IMG_WIDTH, IMG_HEIGHT);
                        recvcount = 0;
                        printf("graymap received\n");
                }
                break;//DEBUG  强制只接收一帧
        }
        printf("socket closed\n");

        close(serv_fd);
        close(connfd);

        //退出线程
        pthread_exit(NULL);

        return 0;
}

//接收语音的线程，等待对方的连接，并接收数据
void recv_sound_routine()
{
        // １，创建TCP套接字
        int fd = socket(AF_INET /*IPv4*/, SOCK_STREAM /*TCP*/, 0);
        char buf[100];
        int ret;
        struct sockaddr_in addr;
        socklen_t len = sizeof(addr);

        addr.sin_family = AF_INET;
        addr.sin_addr.s_addr = inet_addr("192.168.114.100"); // 绑定本机IP
        addr.sin_port = htons(55000);                        // 端口号，范围: 1 - 65535，最好用50000以上的端口防冲突

        // ２，绑定地址（IP + PORT）
        int retb = bind(fd, (struct sockaddr *)&addr, len);
        if (retb == -1)
        {
                perror("bind sound tcp fail");
                close(fd);
                return NULL;
        }
        // ３，将套接字设置为监听状态
        // 设定之后的fd，就是监听套接字，专用来等待连接的
        int retl = listen(fd, 3 /*设定最大同时连接个数*/);
        if (retl == -1)
        {
                perror("listen fail");
                close(fd);
                return NULL;
        }
        // ４，坐等对方的连接请求... ...
        // 返回的connfd，就是已连接套接字，专用来读写通信的
        struct sockaddr_in peer;
        len = sizeof(peer);

        bzero(&peer, len);
        int connfd = accept(fd, (struct sockaddr *)&peer /*获取对方的地址信息*/, &len);
        //接收对方发来的语音消息，
        if (connfd > 0)
        {
                printf("new connection: [%s:%hu]\n", inet_ntoa(peer.sin_addr), ntohs(peer.sin_port));
        }
        else
        {
                perror("accept() failed");
                close(connfd);
                close(fd);
                return NULL;
        }
        // ５，将收到的数据,保存到本地  ubuntu上
        // 创建空白的wav用于保存接收到的内容
        int newwavfd = open("./recv.wav", O_CREAT | O_RDWR | O_TRUNC);
        if (newwavfd == -1)
        {
                perror("create wav failed\n");
                close(fd);
                close(connfd);
                return NULL;
        }
        while (1)
        {
                bzero(buf, 100); // 1000
                ret = recv(connfd, buf, 100, 0);
                if (ret == -1)
                {
                        perror("recv failed");
                        return NULL;
                }
                printf("ubuntu.c 94 recv ret_val = %d\n", ret);
                //趁热吃，放到空白的wav中
                write(newwavfd, buf, ret);
                if (ret < 100)
                {
                        //设置接收消息的标志位
                        // recv_sound_flag = 1;
                        // //1，刷新界面，接收按钮变成红色
                        // show_bmp_bymap(lcdmem, "/root/project/sound_chat/sound.bmp");
                        // //2，提示有语音消息发过来，通过system调用aplay播放提示音
                        // system("aplay /root/project/sound_chat/recv_sound.wav");
                }
        }
        close(newwavfd);
        close(fd);
        close(connfd);

        //退出线程
        pthread_exit(NULL);

        return 0;
}
